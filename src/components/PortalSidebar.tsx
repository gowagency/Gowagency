...
const navItems = [
  ...
  { href: '/portal/calendario', label: 'Calendário Editorial', icon: Calendar },
  { href: '/portal/aprovacao', label: 'Aprovação de Conteúdo', icon: CheckSquare },
  { href: '/portal/documentos', label: 'Documentos', icon: FolderOpen },
  { href: '/portal/insights', label: 'Insights Diários', icon: Lightbulb },
]
...
export function PortalSidebar({ clientName, clientCompany, clientAvatar, onClose }: Props) {
      ...
      <div className="p-6 border-b border-dark-800">
        <Link href="/" className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-dark-500 text-xs">Portal do Cliente</span>
          </div>
        </Link>
      </div>
      ...
      <div className="p-4 border-t border-dark-800 space-y-3">
        <div className="flex items-center gap-3 px-3 py-2">
          <div className="w-8 h-8 bg-gold-500/20 border border-gold-500/30 rounded-full flex items-center justify-center flex-shrink-0">
            <span className="text-gold-400 text-xs font-bold">{clientAvatar}</span>
          </div>
          <div className="min-w-0">
            <p className="text-dark-200 text-sm font-medium truncate">{clientName}</p>
            <p className="text-dark-500 text-xs truncate">{clientCompany}</p>
          </div>
        </div>
        <button
